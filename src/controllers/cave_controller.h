...
class CaveController : public QObject {
 ...
 public:
  ...
  void Create(uint x, uint y, double init_chance, int birth_limit,
              int death_limit) {
    delete cave_;
  ...
  view_size GetSize() { return cave_->GetSize(); }
  void Save(std::string path) { cave_->Save(path); }
  bool Update() { return cave_->Update(); }
  bool GetLive(uint y, uint x) { return cave_->GetLive(y, x); }
  bool SetWall(uint y, uint x) { return cave_->SetWall(y, x); }
  bool SetAir(uint y, uint x) { return cave_->SetAir(y, x); }
 ...
 public slots:
  void SetBirthLimit(int birth_limit) { cave_->SetBirthLimit(birth_limit); }
  void SetDeathLimit(int death_limit) { cave_->SetDeathLimit(death_limit); }
};
