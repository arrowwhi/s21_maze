...
/*!
    \brief Class for creating and updating caves using cellular automaton
generation. In many games there is a need for branching locations, such as
caves. They can be created by generation using the cellular automaton. This kind
of generation uses an idea similar to the Game of Life. The idea of the proposed
algorithm consists of implementing only two steps: first, the whole field is
filled randomly with walls - i.e., for each cell it is randomly determined
whether it will be free or impassable - and then the map state is updated
several times according to the conditions, similar to the birth/death ones in
the Game of Life. The rules are simpler than in the Game of Life - there are two
special variables, one for "birth" of "dead" cells (the "birth" limit) and one
for destruction of "live" cells (the "death" limit). If "live" cells are
surrounded by "live" cells, the number of which is less than the "death" limit,
they "die". In the same way, if "dead" cells are next to "live" cells, the
number of which is greater than the "birth" limit, they become "live".
*/
class Cave {
  uint x_ = 0, y_ = 0;
  bool **cave_;
 ...
 public:
  /*!
      \brief Constructor to create a cave with given parameters and random
     initial state \param x The width of the cave \param y The height of the
     cave \param init_chance The initial probability of a cell being a wall
      \param birth_limit The birth limit for cells
      \param death_limit The death limit for cells
  */
  Cave(uint x, uint y, double init_chance, int birth_limit,
       int death_limit) noexcept;
  /*!
      \brief Constructor to create a cave with given parameters and empty
     initial state \param x The width of the cave \param y The height of the
     cave \param birth_limit The birth limit for cells \param death_limit The
     death limit for cells
  */
  ...
  */
  ...
  /*!
      ...
      \return A constant reference to the solve_stack data member storing the
     path
  */
