...
MazeScene::MazeScene(Ui::Widget *ui) noexcept
    : ui_(ui),
...
void MazeScene::mousePressEvent(QGraphicsSceneMouseEvent *mouseEvent) {
  PathDraw(mouseEvent->scenePos().x() / step_x_,
           mouseEvent->scenePos().y() / step_y_);
}
...
void MazeScene::AddCell(int x, int y, std::pair<bool, bool> cell) {
  if (cell.first)
    addLine(step_x_ * x, step_y_ * (y + 1), step_x_ * (x + 1),
            step_y_ * (y + 1), cell_pen_);
  if (cell.second)
    addLine(step_x_ * (x + 1), step_y_ * y, step_x_ * (x + 1),
            step_y_ * (y + 1), cell_pen_);
}
